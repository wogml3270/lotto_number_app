...
export default async function handler(
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      // API 응답에서 당첨 번호 추출
      const { bnusNo, drwtNo1, drwtNo2, drwtNo3, drwtNo4, drwtNo5, drwtNo6 } =
        lottoApiResponse.data;

      // 로또 번호 리스트 파싱
      const userNumbers = lottoNumbers
        .split(",")
        .map((num: any) => parseInt(num, 10));

      // 일치하는 번호 개수 계산
      const matchedNumbers = userNumbers.filter((num: any) =>
      ...
      if (matchedNumbers.length === 6) {
      ...
      } else if (matchedNumbers.length === 5 && userNumbers.includes(bnusNo)) {
        ...
        result = "꽝입니다. 다음 기회에 다시 시도하세요.${drwtNo1}";
      }
